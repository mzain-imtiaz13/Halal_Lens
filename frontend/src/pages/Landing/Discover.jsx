      ...
      <div className="mx-auto max-w-6xl px-4 py-12 md:py-16">
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {screenshots.map((shot) => (
            <div
              key={shot.title}
              className="rounded-3xl bg-white border border-emerald-100 flex flex-col items-center shadow-sm hover:-translate-y-1 hover:shadow-xl transition-transform duration-300"
            >
              <div className="relative mb-2 w-full overflow-hidden rounded-2xl bg-emerald-50 aspect-auto">
                <img src={shot.src} />
              </div>
              <h3 className="text-sm font-semibold text-emerald-900 text-center">
                {shot.title}
              </h3>
              <p className="text-xs text-emerald-700 text-center p-4">
                {shot.description}
              </p>
            </div>
          ))}
