...
const InfoRow = ({ labelEn, labelJa, children }) => (
  <div className="border-t border-brand-100 first:border-t-0 py-3 md:py-4">
    <div className="grid md:grid-cols-[220px,1fr] gap-2 md:gap-4">
      <div className="text-xs font-semibold tracking-wide text-brand-700 uppercase">
        <div>{labelEn}</div>
        <div className="mt-0.5 text-[11px] normal-case text-brand-500">
          {labelJa}
        </div>
      </div>
      <div className="text-sm text-brand-900 leading-relaxed">{children}</div>
    </div>
  </div>
);
...
const BusinessInfo = () => {
  return (
    <div className="min-h-screen bg-brand-50 text-brand-950 flex items-start justify-center px-4 py-10 md:py-14">
      <article className="w-full max-w-3xl bg-white border border-brand-100 rounded-3xl shadow-sm p-6 md:p-8">
        {/* Title / Intro */}
        <header className="mb-6 md:mb-8 border-b border-brand-100 pb-4">
          <h1 className="text-2xl md:text-3xl font-bold text-brand-950 mb-2">
            Specified Commercial Transactions Act Notice
          </h1>
          <p className="text-sm font-medium text-brand-700">
            特定商取引法に基づく表示
          </p>
          <p className="mt-4 text-sm text-brand-800 leading-relaxed">
        ...
        <section aria-label="Business information">
          ...
          <InfoRow labelEn="Website" labelJa="ウェブサイト">
            <a
              ...
              className="text-brand-800 underline break-all hover:text-brand-600"
            >
              https://halallens.org
            </a>
          </InfoRow>
          ...
          <InfoRow labelEn="Service Provided" labelJa="提供するサービスの内容">
            ...
            <ul className="mt-1 list-disc list-inside space-y-1 text-brand-800">
          ...
          <InfoRow labelEn="Price" labelJa="販売価格">
            ...
            <ul className="mt-2 list-disc list-inside text-brand-800 text-sm">
