    ...
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <div className="flex justify-end gap-2 pt-2">
        ...
        <button
          type="submit"
          disabled={saving}
          className="inline-flex items-center rounded-md bg-emerald-600 px-3 py-1.5 text-xs font-semibold text-white shadow-sm hover:bg-emerald-700 disabled:opacity-60"
        >
          {saving ? "Saving..." : "Save"}
        </button>
      </div>
...
function IngredientRow({ value, onChange, onRemove }) {
  ...
  return (
    <div className="grid gap-2 rounded-md border border-slate-200 bg-slate-50/60 px-3 py-2 text-xs md:grid-cols-[1.1fr,0.9fr,1.3fr,0.6fr,0.6fr,0.6fr,1.4fr,auto]">
      <input
        className="rounded-md border border-slate-300 bg-white px-2 py-1 text-xs focus:outline-none focus:ring-1 focus:ring-emerald-500"
        placeholder="Name"
        value={x.name}
      ...
      <input
        type="number"
        min="0"
        className="rounded-md border border-slate-300 bg-white px-2 py-1 text-right text-xs focus:outline-none focus:ring-1 focus:ring-emerald-500"
        value={x.halalVotes}
        onChange={(e) =>
