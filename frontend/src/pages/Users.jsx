...
export default function Users() {
  ...
  const openHistory = async (userRow) => {
    ...
    try {
      const { items, nextCursor } = await listUserScanHistory({ userId: userRow.id, pageSize: 20 })
      setScans(items)
  ...
  const columns = [
    ...
    { title: 'Active Months', dataIndex: 'active_months' },
    ...
    <>
      ...
      <Toolbar onReset={reset}>
        <input
          ...
          value={q}
          onChange={e => { setPage(1); setCursors([]); setQ(e.target.value) }}
        />
        <select
          className="select"
          value={plan}
          onChange={e => { setPage(1); setCursors([]); setPlan(e.target.value) }}
        >
        ...
        <select
          className="select"
          value={status}
          onChange={e => { setPage(1); setCursors([]); setStatus(e.target.value) }}
        >
        ...
        <>
          ...
          <aside className="drawer">
            <div className="drawer-header">
              <div>
                <div className="drawer-title">Scan History</div>
                {selectedUser && (
                  <div className="helper">{selectedUser.name} • {selectedUser.email}</div>
                )}
              </div>
            ...
            <div className="card" style={{padding:0, overflow:'hidden'}}>
              <table className="table">
                ...
                <tbody>
                  {scans.length === 0 && (
                    <tr><td className="table-empty" colSpan={6}>{scansLoading ? 'Loading...' : 'No scans found'}</td></tr>
                  )}
                  {scans.map(s => (
                    <tr key={s.id}>
                      <td>
                        {s.frontImageUrl
                          ? <img src={s.frontImageUrl} alt="" style={{width:54, height:40, objectFit:'cover', borderRadius:8, border:'1px solid var(--border)'}} />
                          : <span className="helper">—</span>
                        }
                      </td>
                      <td>
                        <div style={{fontWeight:600}}>{s.productName}</div>
                        <div className="helper" style={{fontSize:12}}>{s.productBrands}</div>
                      </td>
                      <td>{s.barcode}</td>
                      <td>
                        <span className={`badge ${s.overallStatus==='halal'?'ok':s.overallStatus==='haram'?'danger':'warn'}`}>
                          {s.overallStatus}
                        </span>
                      </td>
                      <td>{s.scanType}</td>
            ...
            <div className="row" style={{justifyContent:'center'}}>
              ...
              >
                {scansLoading ? 'Loading...' : (scansCursor ? 'Load more' : 'No more')}
              </button>
            </div>
          </aside>
        </>
